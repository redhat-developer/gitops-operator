...
var _ = Describe("GitOps Operator Parallel E2E Tests", func() {
	...
	Context("1-125_validate_applicationset_cleanup_when_spec_field_omitted", func() {
		...
		It("removes ApplicationSet workload and namespaced RBAC when spec.applicationSet is removed from the ArgoCD CR", func() {
			...
			Eventually(depl, "5m", "5s").Should(k8sFixture.ExistByName())

			sa := &corev1.ServiceAccount{
				ObjectMeta: metav1.ObjectMeta{Name: appsetWorkloadName, Namespace: testNS.Name},
			}
			Eventually(sa, "5m", "5s").Should(k8sFixture.ExistByName())

			role := &rbacv1.Role{
				ObjectMeta: metav1.ObjectMeta{Name: appsetWorkloadName, Namespace: testNS.Name},
			}
			Eventually(role, "5m", "5s").Should(k8sFixture.ExistByName())

			rb := &rbacv1.RoleBinding{
				ObjectMeta: metav1.ObjectMeta{Name: appsetWorkloadName, Namespace: testNS.Name},
			}
			Eventually(rb, "5m", "5s").Should(k8sFixture.ExistByName())

			svc := &corev1.Service{
				ObjectMeta: metav1.ObjectMeta{Name: appsetWorkloadName, Namespace: testNS.Name},
			}
			Eventually(svc, "5m", "5s").Should(k8sFixture.ExistByName())
			...
			Eventually(depl, "5m", "5s").Should(k8sFixture.NotExistByName())
			Consistently(depl, "30s", "5s").Should(k8sFixture.NotExistByName())

			Eventually(sa, "5m", "5s").Should(k8sFixture.NotExistByName())
			Consistently(sa, "30s", "5s").Should(k8sFixture.NotExistByName())

			Eventually(role, "5m", "5s").Should(k8sFixture.NotExistByName())
			Consistently(role, "30s", "5s").Should(k8sFixture.NotExistByName())

			Eventually(rb, "5m", "5s").Should(k8sFixture.NotExistByName())
			Consistently(rb, "30s", "5s").Should(k8sFixture.NotExistByName())

			Eventually(svc, "5m", "5s").Should(k8sFixture.NotExistByName())
			Consistently(svc, "30s", "5s").Should(k8sFixture.NotExistByName())
		})
	})
})
