...
`,
		...
		It("hydrate kustomize to another branch via ssh", func() {
			...
			Expect(repo.CheckoutBranch("hydrated")).To(Succeed())
			manifest, err := repo.ReadFile("app/manifest.yaml")
			Expect(err).NotTo(HaveOccurred())
			Expect(manifest).To(ContainSubstring("kind: ConfigMap"))
			Expect(manifest).To(ContainSubstring("name: source-hydrator-test"))
			Expect(manifest).To(ContainSubstring("foo: prod"))
		...
		It("hydrate helm to another directory via https", func() {
			...
			Expect(repo.CheckoutBranch("main")).To(Succeed())
			manifest, err := repo.ReadFile("helm-output/manifest.yaml")
			Expect(err).NotTo(HaveOccurred())
			Expect(manifest).To(ContainSubstring("kind: ConfigMap"))
			Expect(manifest).To(ContainSubstring("name: source-hydrator-helm-test"))
			Expect(manifest).To(ContainSubstring("foo: helm"))
			Expect(manifest).NotTo(ContainSubstring("foo: base"))
		})
	})
})
