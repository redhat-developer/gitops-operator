...
var _ = Describe("GitOps Operator Sequential E2E Tests", func() {
	Context("1-053_validate_argocd_agent_principal_connected_test", func() {
		...
		BeforeEach(func() {
			...
			fixture.SetEnvInOperatorSubscriptionOrDeployment("ARGOCD_CLUSTER_CONFIG_NAMESPACES", "openshift-gitops, ns-hosting-principal, ns-hosting-managed-agent, ns-hosting-autonomous-agent")
		...
		runResourceProxyTest := func(argoEndpoint string, password string, app argocdv1alpha1.Application, agentK8sClient client.Client, agentInstallNamespace string) {
			...
			Eventually(func() bool {
				...
				resource, err := getResourceForResourceProxyTest(argoEndpoint, password, &app,
					"apps", "v1", "Deployment", app.Spec.Destination.Namespace, "spring-petclinic")
				if err != nil {
				...
				return napp.Kind == "Deployment" && napp.Name == "spring-petclinic"
			}, "2m", "5s").Should(BeTrue())

			// Getting a non-existing resource must result in failure
			_, err := getResourceForResourceProxyTest(argoEndpoint, password, &app,
				"apps", "v1", "Deployment", app.Spec.Destination.Namespace, "spring-petclinic-backend")
		...
		runRedisTest := func(argoEndpoint string, password string, managedAgent bool, appOnPrincipal argocdv1alpha1.Application, agentK8sClient client.Client) {
			...
			for idx := range podList.Items {
				...
				if strings.Contains(pod.Name, "spring-petclinic") {
					oldPod = pod
					break
				}
			}
			...
			Eventually(func() bool {
				...
				for idx := range podList.Items {
					pod := podList.Items[idx]
					if strings.Contains(pod.Name, "spring-petclinic") && pod.Name != oldPod.Name {
						newPod = pod
						break
					}
				}
