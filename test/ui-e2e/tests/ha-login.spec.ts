...
test.describe('HA Login Verification', () => {
  ...
  test.beforeAll(async () => {
    test.setTimeout(360000); //6 mins for ha rollout
    
    console.log('\n[setup] Enabling High Availability (HA) for Argo CD...');
    try {
      //patch cr
      execSync('oc patch argocd openshift-gitops -n openshift-gitops --type=merge -p \'{"spec":{"ha":{"enabled":true}}}\'', { stdio: 'inherit' });
  ...
  test.afterAll(async () => {
    ...
    try {
      //disable ha
      execSync('oc patch argocd openshift-gitops -n openshift-gitops --type=merge -p \'{"spec":{"ha":{"enabled":false}}}\'', { stdio: 'inherit' });
      ...
      //wait for ha components to delete
      try {
        execSync('oc wait --for=delete statefulset/openshift-gitops-redis-ha-server -n openshift-gitops --timeout=300s', { stdio: 'ignore' });
        execSync('oc wait --for=delete deployment/openshift-gitops-redis-ha-haproxy -n openshift-gitops --timeout=300s', { stdio: 'ignore' });
      } catch (e) {
